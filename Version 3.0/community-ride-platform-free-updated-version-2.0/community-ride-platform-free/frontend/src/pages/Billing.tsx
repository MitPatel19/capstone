...
export default function Billing() {
  ...
  async function load() {
    const [b, h, s, rides] = await Promise.all([api.get('/billing/me'), api.get('/billing/history'), api.get('/billing/summary'), api.get('/rides/me')])
    setBill(b.data)
    setHistory(h.data)
    setSummary(s.data)
    const completed = (rides.data as Ride[]).filter((r) => r.status === 'completed')
    const uid = Number(localStorage.getItem('crcp_uid') ?? '-1')
    if (role === 'rider' && uid > 0) {
      const joinedRows = await Promise.all(
        completed
          .filter((r) => r.rider_id !== uid)
          .map(async (r) => {
            try {
              const jr = await api.get(`/rides/${r.id}/my_join_request`)
              return { rideId: r.id, from: jr.data?.from_text || '', to: jr.data?.to_text || '' }
            } catch {
              return { rideId: r.id, from: '', to: '' }
            }
          })
      )
      const byRideId = new Map(joinedRows.map((x) => [x.rideId, x]))
      setCompletedRides(
        completed.map((r) => {
