...
export default function RideDetail() {
  ...
  const currentPrice = useMemo(() => (ride ? ride.bargain_price ?? ride.posted_price : 0), [ride])
  ...
  async function load() {
    ...
    try {
      ...
      setRide(r.data)
      const isPrimary = role === 'rider' && Number(r.data?.rider_id) === uid
      if (isPrimary) {
      ...
      if (role === 'rider' && Number(r.data?.rider_id) !== uid) {
        try {
