...
export default function Profile() {
  ...
  async function load() {
    ...
    if (me.data.role === 'rider') {
      try {
        const [cityList, stats] = await Promise.all([api.get('/auth/cities'), api.get('/auth/rider_stats')])
        setCities(cityList.data)
        setRiderStats(stats.data)
      } catch {
        setCities([])
        setRiderStats(null)
      }
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="space-y-4">
            ...
            {isDriver ? (
              ...
              <Card>
                <CardHeader>
                  <div className="font-bold">Ride Statistics</div>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex items-center justify-between text-base">
                    <span className="text-slate-600">Total Rides</span>
                    <span className="text-2xl font-black">{riderStats?.total_rides ?? 0}</span>
                  </div>
                  <div className="flex items-center justify-between text-base">
                    <span className="text-slate-600">This Month</span>
                    <span className="text-2xl font-black">{riderStats?.rides_this_month ?? 0}</span>
                  </div>
                  <div className="text-base text-slate-600">Favorite Route</div>
                  <div className="text-2xl font-semibold">{riderStats?.favorite_route || 'Downtown to Uptown'}</div>
                </CardContent>
              </Card>
            )}
          </div>
              ...
              <CardContent className="space-y-4">
                ...
                {isDriver ? (
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <MapPin className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                      <Input className="pl-10" value={form.default_address} onChange={(e) => setForm((v) => ({ ...v, default_address: e.target.value }))} />
                    </div>
                  </div>
                )}
              </CardContent>
