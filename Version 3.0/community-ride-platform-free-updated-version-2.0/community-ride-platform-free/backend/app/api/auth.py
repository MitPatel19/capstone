...
from app.models import User, UserRole, DriverProfile, DriverApprovalStatus, UserProfile, Rating, Ride, RideStatus, City, DriverCitySelection, RiderDefaultRoute, DriverVehicle
...
def metrics(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if user.role == UserRole.rider:
        total = db.scalar(select(Ride).where(Ride.rider_id==user.id).count()) if False else None
    # SQLAlchemy 2 count workaround:
    rider_total = len(db.scalars(select(Ride.id).where(Ride.rider_id==user.id)).all()) if user.role==UserRole.rider else 0
    rider_active = len(db.scalars(select(Ride.id).where(Ride.rider_id==user.id, Ride.status.notin_([RideStatus.completed, RideStatus.cancelled]))).all()) if user.role==UserRole.rider else 0
...
def rider_stats(user: User = Depends(require_role(UserRole.rider)), db: Session = Depends(get_db)):
    ...
    rides = db.scalars(select(Ride).where(Ride.rider_id==user.id)).all()
    total = len(rides)
    ...
    for r in rides:
        ...
        if r.status == RideStatus.completed:
            # favorite route: pickup -> first stop (or pickup -> (no stop))
            first = None
            if r.stops:
